...
public class RememberCasAuthenticationEntryPoint implements AuthenticationEntryPoint, InitializingBean {
	...
	public void afterPropertiesSet() throws Exception {
        ...
        Assert.hasLength(this.pathLogout, "pathLogin must be specified");
...
*/
        ...
        } if(this.pathLogout.equals(resourcePath)){
            final String redirectUrl = createRedirectUrl(casProperties.getUrl().getLogout() + "?service="+ casProperties.getService().getHome());

            log.debug("app/logout perform CAS logout redirect {}",redirectUrl);
            response.sendRedirect(redirectUrl);

        }else {
            if(response.isCommitted()){
                log.info("Pre-authenticated entry point called. Rejecting access but response is already committed {}",resourcePath);
            }else{
	...
	protected String createServiceUrl(final HttpServletRequest request, final HttpServletResponse response) {
		...
		return CommonUtils.constructServiceUrl(null, response, service, null,null,
    ...
    public void setPathLogout(String pathLogout) {
        this.pathLogout = pathLogout;
    }
